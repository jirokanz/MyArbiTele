...
class _PionexShardWSManager:
    ...
    def _on_error(self, ws, error):
        logger.debug(f"Pionex shard-{self.shard_id} error: {error}")
    ...
    def run(self):
        ...
        while self._running_ref[0]:
            try:
                ...
                logger.debug(f"Pionex shard-{self.shard_id} exception: {e}")
    ...
    def _run_pair(self, pair: str):
        ...
        while self._running:
            try:
                ...
                if self._running:
                    logger.debug(f"Luno WS {pair} error: {e}")
            if self._running:
    ...
    def _run_forever(self):
        ...
        while self.running:
            try:
                ...
                if self.running:
                    sleep_for = backoff.next_delay()
                    logger.debug(f"Hata WS reconnecting in {sleep_for:.1f}s: {e}")
                    time.sleep(sleep_for)
...
class SinegyPublicWSManager:
    ...
    def _connect(self):
        ...
        while self._running:
            try:
                ...
                logger.debug(f"Sinegy WS error: {e}")
            if self._running:
                sleep_for = backoff.next_delay()
                logger.debug(f"Sinegy WS reconnecting in {sleep_for:.1f}s")
                time.sleep(sleep_for)
    ...
    def _on_error(self, ws, error):
        logger.debug(f"Sinegy WS error: {error}")
